...
class HuaweiRouterClient(BaseRouterClient):
    vendor = "huawei"

    def __init__(self, router_url, username, password, router_name=None, **kwargs):
    ...
    def _get_page_tokens(self):
        ...
        if not tokens:
            raise RuntimeError("No CSRF tokens found on Huawei router page")
    ...
    def login(self):
        if not self.password:
            raise RuntimeError("ROUTER_PASSWORD is required for Huawei router authentication")
...
<request>
  ...
  <mode>1</mode>
        ...
        if "<error>" in r.text:
            raise RuntimeError(f"Huawei challenge failed: {r.text}")
        ...
        client_signature_hex = self._hmac_sha256_hex_message(auth_msg, stored_key_hex)
...
<request>
  ...
  <finalnonce>{servernonce}</finalnonce>
        ...
        if "<error>" in r.text:
            raise RuntimeError(f"Huawei authentication failed: {r.text}")
    ...
    def normalize_signal(self, signal, operator_name=None, network_type=None):
        return {
            ...
            "network_type": network_type or "LTE / 4G",
            "operator": operator_name,
            "pci": self.clean_value(signal.get("pci")),
            "cell_id": self.clean_value(signal.get("cell_id")),
            "enodeb_id": self.clean_value(signal.get("enodeb_id")),
            "rsrp": self.clean_value(signal.get("rsrp")),
            "rsrq": self.clean_value(signal.get("rsrq")),
            "sinr": self.clean_value(signal.get("sinr")),
            "rssi": self.clean_value(signal.get("rssi")),
            "band": self.clean_value(signal.get("band")),
            "band_info": self.clean_value(signal.get("bandInfo")),
            "earfcn": self.clean_value(signal.get("earfcn")),
            "ul_bandwidth": self.clean_value(signal.get("ulbandwidth")),
            "dl_bandwidth": self.clean_value(signal.get("dlbandwidth")),
            "tac": self.clean_value(signal.get("tac")),
            "plmn": self.clean_value(signal.get("plmn")),
            "rrc_status": self.clean_value(signal.get("rrc_status")),
            "txpower": self.clean_value(signal.get("txpower")),
            "transmode": self.clean_value(signal.get("transmode")),
            "cqi0": self.clean_value(signal.get("cqi0")),
            "raw_signal": signal,
        }
